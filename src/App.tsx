...
export default function App() {
  ...
  const handleMove = async (move: Move) => {
    ...
    if (move.captures && move.path) {
      ...
      const wasKing = movingPiece?.type === 'king';
      currentBoard = applyMove(state.board, move);
      const newPiece = currentBoard[move.to.row][move.to.col];
      
      if (!wasKing && newPiece?.type === 'king') {
        soundService.playKing();
      } else {
    ...
    const winner = repetitions >= 3 ? 'draw' : (nextMoves.length === 0 ? state.currentPlayer : null);

    if (winner) {
      if (winner === 'blue') soundService.playWin();
      else if (winner === 'yellow') soundService.playLose();
    }
